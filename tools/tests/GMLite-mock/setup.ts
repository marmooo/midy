// GMLite test setup. Re-exports shared utilities and defines the PlayerFactory type.
import { Channel, MidyGMLite, Note } from "../../src/midy-GMLite.ts";
import {
  AnyPlayer,
  assertAlmostEquals,
  assertEquals,
...
export { Channel, MidyGMLite, Note };
...
// -------------------------------------------------------------------------
// GMLite player factory
// -------------------------------------------------------------------------

/**
 * Build a MidyGMLite instance wired up for unit testing.
 * `getAudioBuffer` is stubbed so tests never touch the real decoder.
 */
export function setupGMLitePlayer(exclusiveClass = 0): MidyGMLite {
...
export const gmliteFactory: PlayerFactory = (exclusiveClass = 0) =>
  setupGMLitePlayer(exclusiveClass) as unknown as BasicPlayer;

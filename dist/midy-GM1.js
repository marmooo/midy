...
var MidyGM1 = class extends EventTarget {
  ...
  stopActiveNotes(channelNumber, velocity, force, scheduleTime) {
    ...
    const promises = [];
    this.processActiveNotes(channel2, scheduleTime, (note) => {
  ...
  async processActiveNotes(channel2, scheduleTime, callback) {
    const scheduledNotes = channel2.scheduledNotes;
    const tasks = [];
    for (let i = channel2.scheduleIndex; i < scheduledNotes.length; i++) {
      const note = scheduledNotes[i];
  ...
  updateDetune(channel2, note, scheduleTime) {
    note.bufferSource.detune.cancelScheduledValues(scheduleTime).setValueAtTime(channel2.detune, scheduleTime);
  }
  setVolumeEnvelope(note, scheduleTime) {
    ...
    note.volumeEnvelopeNode.gain.cancelScheduledValues(scheduleTime).setValueAtTime(0, startTime).setValueAtTime(0, volDelay).linearRampToValueAtTime(attackVolume, volAttack).setValueAtTime(attackVolume, volHold).setTargetAtTime(sustainVolume, volHold, decayDuration * decayCurve);
  }
  setPitchEnvelope(note, scheduleTime) {
    const { voiceParams } = note;
    const baseRate = voiceParams.playbackRate;
    note.bufferSource.playbackRate.cancelScheduledValues(scheduleTime).setValueAtTime(baseRate, note.startTime);
  ...
  setFilterEnvelope(note, scheduleTime) {
    ...
    note.filterNode.frequency.cancelScheduledValues(scheduleTime).setValueAtTime(adjustedBaseFreq, startTime).setValueAtTime(adjustedBaseFreq, modDelay).linearRampToValueAtTime(adjustedPeekFreq, modAttack).setValueAtTime(adjustedPeekFreq, modHold).setTargetAtTime(
      adjustedSustainFreq,
      modHold,
  ...
  createMessageHandlers() {
    ...
    handlers[128] = (data, scheduleTime) => this.noteOff(data[0] & 15, data[1], data[2], scheduleTime);
    handlers[144] = (data, scheduleTime) => this.noteOn(data[0] & 15, data[1], data[2], scheduleTime);
    handlers[176] = (data, scheduleTime) => this.setControlChange(data[0] & 15, data[1], data[2], scheduleTime);
    handlers[192] = (data, scheduleTime) => this.setProgramChange(data[0] & 15, data[1], scheduleTime);
    handlers[224] = (data, scheduleTime) => this.handlePitchBendMessage(
      data[0] & 15,
      data[1],
      data[2],
      scheduleTime
  ...
  setModLfoToPitch(channel2, note, scheduleTime) {
    if (note.modulationDepth) {
      ...
      note.modulationDepth.gain.cancelScheduledValues(scheduleTime).setValueAtTime(depth, scheduleTime);
    } else {
